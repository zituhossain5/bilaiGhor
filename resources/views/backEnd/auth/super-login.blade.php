...
<head>
    ...
    <title>Super Admin | {{ optional($generalsetting)->name ?? config('app.name') }}</title>
    @php
...
<body>
    <div class="shell">
        ...
        <h1>{{ optional($generalsetting)->name ?? 'Control Panel' }}</h1>
        ...
        <div class="card">
            <div class="brand">
                @if($logo)
                    <img src="{{ $logo }}" alt="">
                @else
                    <div class="brand-fallback">{{ optional($generalsetting)->name ?? config('app.name') }}</div>
                @endif
            </div>
