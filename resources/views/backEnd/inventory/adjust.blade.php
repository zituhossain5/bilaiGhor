...
<div class="container-fluid mb-5">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4 mt-2">
        ...
        <a href="{{ route('admin.inventory.stock') }}" class="d-none d-sm-inline-block btn btn-sm btn-light shadow-sm">
    ...
    <div class="row">
        <div class="col-lg-5 mb-4">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="GET" action="{{ route('admin.inventory.adjust') }}">
                        <label class="form-label">Product</label>
                        <div class="d-flex gap-2">
                            <select name="product_id" class="form-select">
                                <option value="">Choose a product…</option>
                                @foreach($products as $p)
                                    <option value="{{ $p->id }}" @selected(optional($selected)->product_id == $p->id)>{{ Str::limit($p->name, 56) }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row text-center">
                        <div class="col-4">
                            ...
                            <div class="stats-value" id="inv-cur-onhand">{{ $selected->on_hand }}</div>
                        </div>
                        <div class="col-4">
                            <div class="stats-label text-info">Reserved</div>
                            <div class="stats-value">{{ $selected->reserved }}</div>
                        </div>
                        <div class="col-4">
                            <div class="stats-label text-success">Available</div>
                            <div class="stats-value">{{ $selected->available }}</div>
                        </div>
                    </div>
                    @endif
                </div>
        ...
        <div class="col-lg-7 mb-4">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @else
                    <form method="POST" action="{{ route('admin.inventory.adjust.store') }}" id="invAdjustForm">
                        @csrf
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <textarea name="notes" class="form-control" rows="2" maxlength="1000" placeholder="e.g. 2 damaged packets found during shelf count">{{ old('notes') }}</textarea>
                            </div>
                            <div class="col-12">
                                <div class="alert alert-secondary py-2 mb-2">
                                    Preview: <strong id="inv-preview-cur">{{ $selected->on_hand }}</strong>
                                    <span id="inv-preview-op">→</span>
                                    New On Hand: <strong id="inv-preview-new">{{ $selected->on_hand }}</strong>
