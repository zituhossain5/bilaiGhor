                                            ...
                                            <div class="form-group mb-0 mb-md-2">
                                                ...
                                                <select name="division_id" id="checkout_division" class="form-control-custom" required>
                                                    ...
                                                    @foreach(($divisions ?? collect()) as $d)
                                                        <option value="{{ $d->id }}" @selected((string) $selDivision === (string) $d->id)>{{ $d->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                ...
                <div class="col-lg-5 col-md-12 cust-order-1">
                    <div class="sticky-sidebar">
                        ...
                        <div class="checkout-card">
                            ...
                            {{-- Reward earn info (placeholder: reward system not yet implemented) --}}
                        ...
                        {{-- CARD 2: Use Your Reward Point (placeholder UI, disabled) --}}
                        <div class="checkout-card">
                            <div class="checkout-header">
                                {{-- Replace SVG icon later --}}
                                <i class="fa fa-star"></i>
                            ...
                            <div class="card-body-custom">
                                <div class="bilai-co-reward-row">
                                    ...
                                    {{-- Toggle disabled: reward redemption not yet implemented (safe placeholder) --}}
...
@if($requires_shipping)
{{-- ── Dependent dropdown preselect for logged-in prefill (district/upazila load async) ── --}}
@php
    $__prefillJs = [
...
$(function () {
    ...
    $.get('{{ url('/ajax/delivery/districts') }}/' + PF.division_id, function (res) {
        var opts = '<option value="">জেলা নির্বাচন করুন</option>';
        (res.data || []).forEach(function (r) {
            opts += '<option value="' + r.id + '" data-charge="' + r.delivery_charge + '">' + r.name + ' (৳' + r.delivery_charge + ')</option>';
        });
        ...
        if (PF.free) {
            ...
            $.get('{{ route("shipping.charge") }}', { id: $dist.val() });
        }
        ...
        $.get('{{ url('/ajax/delivery/upazilas') }}/' + PF.district_id, function (res2) {
