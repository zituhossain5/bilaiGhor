...
<div class="bilai-otp-page">
    <div class="bilai-otp-card">
        ...
        <p class="bilai-otp-sub">We sent a {{ $otpLength }}-digit code to {{ $masked }}. It expires in 5 minutes.</p>

        <form action="{{ route('customer.forgot.otp.verify') }}" method="POST" id="bilai-otp-form">
            @csrf
            {{-- The boxes are UI only; this hidden field carries the joined code. --}}
            ...
            @error('otp')<p class="bilai-otp-err">{{ $message }}</p>@enderror

            <button type="submit" class="bilai-otp-btn">Verify OTP</button>
        </form>

        <div class="bilai-otp-resend">
            <form action="{{ route('customer.forgot.otp.resend') }}" method="POST" id="bilai-otp-resend-form">
                @csrf
        ...
        <a href="{{ route('customer.login') }}" class="bilai-otp-back">
