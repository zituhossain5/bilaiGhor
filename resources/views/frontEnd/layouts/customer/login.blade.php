...
<section class="ba-section">
    <div class="ba-card">
        ...
        <div class="ba-tabs">
            <a href="{{ route('customer.login') }}" class="ba-tab active">Login</a>
            <a href="{{ route('customer.register') }}" class="ba-tab">Register</a>
        </div>
        ...
        <form action="{{ route('customer.signin') }}" method="POST">
            ...
            <div class="ba-field">
                ...
                <input type="text" id="login" name="login"
                       class="ba-input no-icon @error('login') is-invalid @enderror"
                       value="{{ old('login') }}"
                       placeholder="email or 017xxxxxxxx" required>
                @error('login')<span class="ba-err">{{ $message }}</span>@enderror
            </div>
            ...
            <div class="ba-row">
                ...
                @if(Route::has('customer.forgot.password'))
                <a href="{{ route('customer.forgot.password') }}" class="ba-forgot">Forgot Password?</a>
                @endif
            </div>
