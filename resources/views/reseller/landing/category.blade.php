...
<head>
    ...
    <title>{{ $category->name }} | {{ $landing->title }}</title>
    ...
    <div class="container mx-auto px-4 py-6 flex flex-col lg:flex-row gap-6">
            ...
            <ul class="space-y-1 text-sm font-medium">
                @foreach($categories ?? [] as $cat)
                    <li class="border-b border-gray-100 last:border-0">
                        <div class="flex items-center">
                            <a href="{{ $categoryRoute($cat) }}" class="flex-1 flex items-center py-3 hover:text-blue-600 {{ ($category->id ?? null) == $cat->id ? 'text-blue-600 font-bold' : '' }}">
                        ...
                        @if($cat->subcategories && $cat->subcategories->count() > 0)
                            <ul id="cat-{{ $cat->id }}" class="ml-11 mb-2 space-y-0.5 {{ ($category->id ?? null) == $cat->id ? '' : 'hidden' }}">
                                @foreach($cat->subcategories as $sub)
                                    <li><a href="{{ $subcategoryRoute($sub) }}" class="block py-1.5 px-2 rounded hover:bg-blue-50 hover:text-blue-600 text-xs">{{ $sub->subcategoryName ?? $sub->name }}</a></li>
                                @endforeach
                            </ul>
                        @endif
                    </li>
        ...
        <main class="flex-1">
            <div class="bg-white rounded-xl shadow p-4 md:p-6 mb-6">
                <h1 class="text-xl md:text-2xl font-bold text-gray-800 border-b-4 border-blue-600 inline-block pb-1">{{ $category->name }}</h1>
        ...
        <ul class="py-2">
            @foreach($categories ?? [] as $cat)
                <li class="border-b">
                    ...
                    @if($cat->subcategories && $cat->subcategories->count() > 0)
                        <button type="button" onclick="event.stopPropagation();toggleSubcat('mcat-{{ $cat->id }}',this)" class="p-3"><i class="fa-solid fa-chevron-right text-xs"></i></button>
                    @endif</div>
                    @if($cat->subcategories && $cat->subcategories->count() > 0)
                        <ul id="mcat-{{ $cat->id }}" class="bg-gray-50 hidden">
                            @foreach($cat->subcategories as $sub)
