...
?>
<?php $__env->startSection('title', 'Order Process #' . $data->invoice_id); ?>
...
<div class="container-fluid order-process-shell">
    ...
    <div class="op-page-header">
        <div>
            ...
            <div class="op-sub">
                ইনভয়েস <strong>#<?php echo e($data->invoice_id); ?></strong>
                · স্ট্যাটাস: <span class="op-badge-status"><?php echo e($currentStatusName); ?></span>
        ...
        <div class="op-header-actions">
            <a href="<?php echo e(route('admin.order.edit', $data->invoice_id)); ?>" class="btn btn-sm btn-outline-primary btn-op-outline">
                <i class="fas fa-edit me-1"></i> এডিট
            </a>
            <a href="<?php echo e(route('admin.order.invoice', $data->invoice_id)); ?>" class="btn btn-sm btn-light btn-op-outline" target="_blank">
                <i class="fas fa-file-invoice me-1"></i> ইনভয়েস
            </a>
            <a href="<?php echo e(route('admin.orders', 'pending')); ?>" class="btn btn-sm btn-outline-secondary btn-op-outline">
    ...
    <div class="op-card op-card-items mb-3">
        <div class="op-card-head">
                    ...
                    <span class="op-badge-invoice"><?php echo e($data->orderdetails->count()); ?> টি পণ্য</span>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover op-items-table mb-0">
                            ...
                            <tbody>
                                ...
                                ?>
                                <tr>
                                    <td><span class="op-sl-badge"><?php echo e($key + 1); ?></span></td>
                                    <td>
                                        <img class="op-product-img" src="<?php echo e(asset($product->image->image ?? 'public/no-image.png')); ?>" alt="">
                                    </td>
                                    <td><strong><?php echo e($product->product_name); ?></strong></td>
                                    <td><?php echo e(($product->color && $product->color->name) ? $product->color->name : ($product->product_color ?: 'N/A')); ?></td>
                                    <td><?php echo e($sizeDisplay); ?></td>
                                    <td class="text-end">৳<?php echo e(number_format($displayPrice, 2)); ?></td>
                                    <td class="text-center">
                                        <span class="badge bg-light text-dark border"><?php echo e($product->qty); ?></span>
                                    </td>
                                    <td class="text-end fw-semibold">৳<?php echo e(number_format($displayPrice * $product->qty, 2)); ?></td>
                                </tr>
    ...
    <form action="<?php echo e(route('admin.order_change')); ?>" method="POST" data-parsley-validate name="editForm" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
                    ...
                    <div class="op-card-body">
                        <div class="row g-3">
                            <div class="col-md-6 op-input-group">
                                ...
                                <input type="text" id="name" class="form-control" name="name" value="<?php echo e($shipRow->name ?? ''); ?>" placeholder="নাম" required>
                            </div>
                            <div class="col-md-6 op-input-group">
                                <label class="op-form-label" for="phone"><i class="fas fa-phone"></i> মোবাইল</label>
                                <input type="text" id="phone" class="form-control" name="phone" value="<?php echo e($shipRow->phone ?? ''); ?>" placeholder="01XXXXXXXXX" required>
                            </div>
                            <div class="col-12 op-input-group">
                                <label class="op-form-label" for="address"><i class="fas fa-map-marker-alt"></i> ঠিকানা</label>
                                <textarea id="address" name="address" class="form-control" rows="2" placeholder="বিস্তারিত ঠিকানা" required><?php echo e($shipRow->address ?? ''); ?></textarea>
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12 op-input-group">
                                ...
                                <select name="division_id" id="adm_process_division" class="form-select" required>
                                    ...
                                    <?php $__currentLoopData = ($divisions ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $d): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($d->id); ?>" <?php echo e($admSelDiv === (int) $d->id ? 'selected' : ''); ?>><?php echo e($d->name); ?></option>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </select>
                            </div>
                            <div class="col-md-6 op-input-group">
                                ...
                                <select name="district_id" id="adm_process_district" class="form-select" required>
                                    <option value=""><?php echo e($admSelDiv ? 'জেলা নির্বাচন করুন' : 'আগে বিভাগ সিলেক্ট করুন'); ?></option>
                                    <?php $__currentLoopData = $processDistricts->where('division_id', $admSelDiv); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $district): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($district->id); ?>" <?php echo e($admSelDist === (int) $district->id ? 'selected' : ''); ?>>
                            ...
                            <div class="col-md-6 op-input-group">
                                ...
                                <select name="upazila_id" id="adm_process_upazila" class="form-select" required>
                                    <option value=""><?php echo e($admSelDist ? 'উপজেলা নির্বাচন করুন' : 'আগে জেলা সিলেক্ট করুন'); ?></option>
                                    <?php $__currentLoopData = $processUpazilas->where('district_id', $admSelDist); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $upazila): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($upazila->id); ?>" <?php echo e($admSelUp === (int) $upazila->id ? 'selected' : ''); ?>>
                        ...
                        <div class="op-input-group">
                            ...
                            <select name="delivery_boy_id" id="delivery_boy_id" class="form-select">
                                ...
                                <?php $__currentLoopData = ($deliveryBoys ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $b): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($b->id); ?>" <?php if($selDelBoy === (int) $b->id): echo 'selected'; endif; ?>>
                                        <?php echo e($b->name); ?> — <?php echo e($b->phone); ?> (৳<?php echo e(number_format($b->commission_per_delivery, 0)); ?>/ডেলিভারি)
                                    </option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        ...
                        <div class="op-card-body">
                            ...
                            <div class="op-summary-grid">
                                <div class="op-summary-row">
                                    <span>সাবটোটাল</span>
                                    <span class="op-val">৳<?php echo e(number_format($subtotal, 2)); ?></span>
                                </div>
                                <div class="op-summary-row">
                                    <span>ডেলিভারি</span>
                                    <span class="op-val">৳<?php echo e(number_format($shipping, 2)); ?></span>
                                </div>
                                <div class="op-summary-row">
                                    <span>ছাড়</span>
                                    <span class="op-val op-val-discount">−৳<?php echo e(number_format($discount, 2)); ?></span>
                                </div>
                                <div class="op-summary-row op-total">
                                    <span><?php echo e($isResellerOrder ? 'গ্রাহক প্রদেয়' : 'মোট'); ?></span>
                                    <span class="op-val">৳<?php echo e(number_format($finalTotal, 2)); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="op-card-body op-payment-panel">
                            <div class="mb-3">
                                ...
                                <?php if(!empty($payGateway)): ?>
                                    <span class="op-gateway-pill"><?php echo e(strtoupper($payGateway)); ?></span>
                                <?php else: ?>
                            ...
                            <div>
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    <select id="payment_status_<?php echo e($data->id); ?>" class="form-select form-select-sm">
                                        <option value="pending" <?php echo e($payStatusVal == 'pending' ? 'selected' : ''); ?>>Pending</option>
                                        <option value="paid" <?php echo e($payStatusVal == 'paid' ? 'selected' : ''); ?>>Paid</option>
                                        <option value="unpaid" <?php echo e($payStatusVal == 'unpaid' ? 'selected' : ''); ?>>Unpaid</option>
                                        <option value="failed" <?php echo e($payStatusVal == 'failed' ? 'selected' : ''); ?>>Failed</option>
                                    </select>
                                    <button type="button" class="btn btn-success btn-sm" onclick="updatePaymentStatus(<?php echo e($data->id); ?>)">
                                        <i class="fa fa-check"></i> আপডেট
                                    </button>
                                </div>
                            </div>
                            <?php echo $__env->make('backEnd.order.partials.manual_payment_verify_box', ['payment' => $payRelation], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                        </div>
                    </div>
...
<script>
(function () {
    var admSelDiv = <?php echo e($admSelDiv); ?>;
    var admSelDist = <?php echo e($admSelDist); ?>;
    var admSelUp = <?php echo e($admSelUp); ?>;
    var allDistricts = <?php echo json_encode($processDistricts->values(), 15, 512) ?>;
    var allUpazilas = <?php echo json_encode($processUpazilas->values(), 15, 512) ?>;
...
<?php echo $__env->make('backEnd.layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\projects\bilaiGhor\resources\views/backEnd/order/process.blade.php ENDPATH**/ ?>