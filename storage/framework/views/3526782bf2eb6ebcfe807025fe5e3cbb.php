...
<?php $__env->startSection('title', '#'.$order->invoice_id); ?>
<?php $__env->startSection('header_title', 'অর্ডার #'.$order->invoice_id); ?>
...
<div class="d-card">
    ...
    <div class="d-cust-name"><?php echo e($order->shipping->name ?? '—'); ?></div>
    <div class="d-phone"><?php echo e($order->shipping->phone ?? ''); ?></div>
    <div class="d-addr"><?php echo e($order->shipping->address ?? ''); ?></div>
    <?php if($customerPhone): ?>
        <a href="tel:<?php echo e(preg_replace('/[^0-9+]/', '', $customerPhone)); ?>" class="d-detail-call-btn">
...
<div class="d-card">
    ...
    <div class="d-order-summary">
        <div class="d-summary-item">
            <span>ইনভয়েস</span>
            <strong>#<?php echo e($order->invoice_id); ?></strong>
        </div>
        <div class="d-summary-item">
            <span>স্ট্যাটাস</span>
            <strong><?php echo e($order->status->name ?? '—'); ?></strong>
        </div>
        <div class="d-summary-item">
            <span>পেমেন্ট</span>
            <strong><?php echo e(strtoupper($paymentMethod)); ?> · <?php echo e(ucfirst($paymentStatus)); ?></strong>
        </div>
        <div class="d-summary-item">
            <span>শিপিং চার্জ</span>
            <strong>৳<?php echo e(number_format((float) ($order->shipping_charge ?? 0), 2)); ?></strong>
        </div>
        <div class="d-summary-item d-summary-item--highlight">
            <span>কাস্টমার থেকে কালেক্ট</span>
            <strong>৳<?php echo e(number_format($collectAmount, 2)); ?></strong>
        </div>
    </div>
</div>
...
<div class="d-card d-card--flush">
    ...
    <?php $__empty_1 = true; $__currentLoopData = $order->orderdetails; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        ...
        <div class="d-product-row">
            <img src="<?php echo e(asset($imagePath)); ?>" alt="" class="d-product-img" width="64" height="64">
            <div class="d-product-info">
                <div class="d-product-name"><?php echo e($item->product_name); ?></div>
                <div class="d-product-meta">
                    <span>Qty: <?php echo e($item->qty); ?></span>
                    <span>Size: <?php echo e($sizeName); ?></span>
                    <span>Color: <?php echo e($colorName); ?></span>
                </div>
                <div class="d-product-price">
                    <span>৳<?php echo e(number_format((float) $item->sale_price, 2)); ?> × <?php echo e($item->qty); ?></span>
                    <strong>৳<?php echo e(number_format($lineTotal, 2)); ?></strong>
                </div>
            </div>
        </div>
...
<?php if(!$order->rider_delivered_at && !$isCancelled): ?>
    <div class="d-card d-otp-card">
        <div class="d-section-title">ডেলিভারি OTP</div>
        <?php if($deliveryOtpPending): ?>
            <p class="d-otp-text">
                কাস্টমারের ফোনে OTP পাঠানো হয়েছে। কোডটি নিয়ে নিচে বসান।
                <br>
                <small>মেয়াদ: <?php echo e($order->delivery_otp_expires_at->format('h:i A')); ?></small>
            </p>
        <?php else: ?>
            <p class="d-otp-text">ডেলিভারি সম্পন্ন করার আগে কাস্টমারের ফোনে OTP পাঠাতে হবে।</p>
        <?php endif; ?>

        <form method="post" action="<?php echo e(route('delivery.orders.deliver', $order->id)); ?>" class="d-form">
            <?php echo csrf_field(); ?>
            <?php if($deliveryOtpPending): ?>
                ...
                <input
                    ...
                    value="<?php echo e(old('delivery_otp')); ?>"
                    required
                >
        ...
        <?php if($deliveryOtpPending): ?>
            <form method="post" action="<?php echo e(route('delivery.orders.deliver', $order->id)); ?>" style="margin-top:10px;">
                <?php echo csrf_field(); ?>
    ...
    <form method="post" action="<?php echo e(route('delivery.orders.deliver', $order->id)); ?>" onsubmit="return confirm('ডেলিভারি সম্পন্ন হিসেবে চিহ্নিত করবেন?');" style="display:none;">
        <?php echo csrf_field(); ?>
    ...
    <div class="d-card d-cancel-card">
        ...
        <form method="post" action="<?php echo e(route('delivery.orders.cancel', $order->id)); ?>" onsubmit="return confirm('এই অর্ডার ক্যানসেল করবেন?');">
            <?php echo csrf_field(); ?>
            <label class="d-label" for="cancel_note">কারণ (ঐচ্ছিক)</label>
            <textarea id="cancel_note" name="cancel_note" rows="2" placeholder="যেমন: কাস্টমার ফোন ধরেনি / নিতে চায়নি"><?php echo e(old('cancel_note')); ?></textarea>
...
<?php else: ?>
    <div class="d-card d-done-banner">
        ডেলিভারি সম্পন্ন — <?php echo e($order->rider_delivered_at->format('d M Y, h:i A')); ?>
...
<?php echo $__env->make('delivery.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bilaiGhor\resources\views\delivery\orders\show.blade.php ENDPATH**/ ?>