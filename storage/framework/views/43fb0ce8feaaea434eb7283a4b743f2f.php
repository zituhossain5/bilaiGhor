...
<head>
    ...
    <title><?php echo e($product->name); ?> | <?php echo e($landing->title); ?></title>
...
<body class="bg-gray-100 font-sans text-gray-800 pb-16 md:pb-0">
    ...
    <header class="bg-white shadow-sm sticky top-0 z-40">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center gap-4">
            <a href="<?php echo e(landing_url($landing->slug, '')); ?>">
                <?php if($landing->logo): ?><img src="<?php echo e(asset($landing->logo)); ?>" alt="<?php echo e($landing->title); ?>" class="h-10 md:h-12 w-auto max-w-[180px] object-contain">
                <?php else: ?><span class="text-2xl font-extrabold text-blue-700"><?php echo e(Str::limit($landing->title ?? 'স্টোর', 20)); ?></span><?php endif; ?>
            </a>
            <form action="<?php echo e(route('home')); ?>" method="GET" class="hidden md:flex max-w-xl flex-1 mx-4">
        ...
        <aside class="w-full lg:w-64 flex-shrink-0 product-sidebar order-2 lg:order-1">
            <div class="bg-white shadow rounded-lg overflow-hidden">
                ...
                <div id="productCatList" class="border-t border-gray-200 max-h-[70vh] overflow-y-auto">
                    <ul class="py-2 text-sm">
                        <?php $__currentLoopData = $categories ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="border-b border-gray-100 last:border-0">
                                <div class="flex items-center">
                                    <a href="<?php echo e($categoryRoute($cat)); ?>" class="flex-1 flex items-center px-4 py-3 hover:bg-blue-50 hover:text-blue-600 transition min-w-0">
                                    ...
                                    <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                                        <button type="button" onclick="toggleSubcat('pcat-<?php echo e($cat->id); ?>', this)" class="p-2 text-gray-400 hover:text-blue-600 flex-shrink-0">
                                            <i class="fa-solid fa-chevron-right text-xs transition-transform duration-200"></i>
                                        </button>
                                    <?php endif; ?>
                                </div>
                                <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                                    <ul id="pcat-<?php echo e($cat->id); ?>" class="bg-gray-50 hidden">
                                        <?php $__currentLoopData = $cat->subcategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <li><a href="<?php echo e($subcategoryRoute($sub)); ?>" class="block py-2 px-4 pl-12 hover:bg-blue-50 hover:text-blue-600 text-xs"><?php echo e($sub->subcategoryName ?? $sub->name); ?></a></li>
                    ...
                    <a href="<?php echo e(landing_url($landing->slug, '')); ?>" class="block px-4 py-3 text-blue-600 font-semibold text-sm hover:bg-blue-50 border-t">হোমে ফিরে যান <i class="fa-solid fa-angle-right ml-1"></i></a>
                </div>
            </div>
        </aside>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 p-4 sm:p-6 product-grid">
                    ...
                    <div>
                        <div id="mainImgWrap" class="aspect-square bg-gray-100 rounded-lg overflow-hidden mb-4 w-full">
                            <img id="mainProductImg" src="<?php echo e(asset($mainImg)); ?>" alt="<?php echo e($product->name); ?>" class="product-responsive-img w-full h-full object-contain">
                        </div>
                        <?php if(count($gallery) > 1): ?>
                            <div class="flex gap-2 overflow-x-auto pb-2">
                                <?php $__currentLoopData = $gallery; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $gimg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <button type="button" onclick="document.getElementById('mainProductImg').src='<?php echo e(asset($gimg)); ?>'" class="flex-shrink-0 w-16 h-16 rounded border-2 border-gray-200 overflow-hidden hover:border-blue-500 focus:border-blue-500">
                                        <img src="<?php echo e(asset($gimg)); ?>" alt="" class="w-full h-full object-cover">
                                    </button>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        <?php endif; ?>
                    ...
                    <div>
                        <h1 class="text-base sm:text-xl md:text-2xl font-bold text-gray-800 mb-2 break-words"><?php echo e($product->name); ?></h1>
                        <?php if($product->brand): ?>
                            <p class="text-sm text-gray-500 mb-3">ব্র্যান্ড: <?php echo e($product->brand->name); ?></p>
                        <?php endif; ?>
                        ...
                        <div class="bg-blue-50 border-2 border-blue-200 rounded-xl p-4 sm:p-5 mb-4 sm:mb-6">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-2xl font-bold text-blue-700">৳<?php echo e(number_format($displayPrice, 0)); ?></span>
                            </div>
                        </div>

                        <?php if($product->stock !== null): ?>
                            <p class="text-sm mb-4">
                                স্টক: <span class="font-semibold <?php echo e($product->stock > 0 ? 'text-green-600' : 'text-red-600'); ?>"><?php echo e($product->stock > 0 ? $product->stock . ' পিস' : 'স্টক আউট'); ?></span>
                            </p>
                        <?php endif; ?>

                        <div class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3">
                            <form action="<?php echo e(landing_url($landing->slug, 'cart/add')); ?>" method="POST" class="inline">
                                <?php echo csrf_field(); ?>
                            ...
                            <?php if($landing->phone): ?>
                                <a href="tel:<?php echo e($landing->phone); ?>" class="inline-flex items-center justify-center px-4 sm:px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition text-sm sm:text-base w-full sm:w-auto">
                                    <i class="fa-solid fa-phone mr-2"></i> কল করুন
                                </a>
                            <?php endif; ?>
                            <a href="javascript:void(0)" onclick="navigator.clipboard.writeText('<?php echo e(route('product', $product->slug)); ?>'); alert('লিংক কপি হয়েছে!');" class="inline-flex items-center justify-center px-4 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition text-sm w-full sm:w-auto">
                ...
                <div class="mt-6 sm:mt-8">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                        <?php $__currentLoopData = $relatedProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            ?>
                            <a href="<?php echo e($productRoute($rp)); ?>" class="bg-white rounded-lg shadow hover:shadow-xl transition p-2 sm:p-3 block">
                                <img src="<?php echo e(asset($rImg)); ?>" alt="" class="w-full h-28 sm:h-36 object-cover rounded mb-2 max-w-full">
                                <h3 class="font-semibold text-sm line-clamp-2"><?php echo e($rp->name); ?></h3>
                                <div class="mt-1">
                                    <span class="font-bold text-blue-700">৳<?php echo e(number_format($rDisplayPrice, 0)); ?></span>
                                </div>
                            </a>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
    ...
    <footer class="bg-gray-900 text-gray-300 pt-6 sm:pt-8 pb-6 mt-8 sm:mt-10">
        <div class="container mx-auto px-4 text-center text-xs sm:text-sm">
            &copy; <?php echo e(date('Y')); ?> <?php echo e($landing->title); ?>. Powered by <?php echo e(config('app.name')); ?>.
        </div>
    </footer>
    ...
    <div id="categoryDrawer" class="fixed top-0 left-0 w-3/4 max-w-[300px] h-full bg-white z-[70] transform -translate-x-full transition-transform shadow-2xl overflow-y-auto">
        ...
        <ul class="py-2">
            <?php $__currentLoopData = $categories ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <li class="border-b">
                    <div class="flex"><a href="<?php echo e($categoryRoute($cat)); ?>" onclick="toggleCategoryMenu()" class="flex-1 py-3 px-4"><?php echo e($cat->name); ?></a>
                    <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                        <button type="button" onclick="event.stopPropagation();toggleSubcat('mcat-<?php echo e($cat->id); ?>',this)" class="p-3"><i class="fa-solid fa-chevron-right text-xs" style="<?php echo e(($product->category_id ?? null) == $cat->id ? 'transform:rotate(90deg)' : ''); ?>"></i></button>
                    <?php endif; ?></div>
                    <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                        <ul id="mcat-<?php echo e($cat->id); ?>" class="bg-gray-50 <?php echo e(($product->category_id ?? null) == $cat->id ? '' : 'hidden'); ?>">
                            <?php $__currentLoopData = $cat->subcategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li><a href="<?php echo e($subcategoryRoute($sub)); ?>" onclick="toggleCategoryMenu()" class="block py-2 px-4 pl-12 text-sm"><?php echo e($sub->subcategoryName ?? $sub->name); ?></a></li>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </ul>
                    <?php endif; ?>
                </li>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <li><a href="<?php echo e(landing_url($landing->slug, '')); ?>" onclick="toggleCategoryMenu()" class="block px-4 py-3 text-blue-600 font-bold">হোম</a></li>
        </ul>
    </div>

    <div class="fixed bottom-0 left-0 w-full bg-white border-t z-50 md:hidden py-2 flex justify-around text-gray-500 text-xs">
        <a href="<?php echo e(landing_url($landing->slug, '')); ?>" class="flex flex-col items-center text-blue-600"><i class="fa-solid fa-house text-lg mb-1"></i>হোম</a>
        ...
        <a href="<?php echo e(landing_url($landing->slug, 'order')); ?>" class="flex flex-col items-center text-green-600"><i class="fa-solid fa-cart-shopping text-lg mb-1"></i>অর্ডার</a>
        <a href="<?php echo e(route('reseller.landing.order-track', $landing->slug)); ?>" class="flex flex-col items-center"><i class="fa-solid fa-truck-fast text-lg mb-1"></i>অর্ডার ট্রাকিং</a>
    </div>
...
<?php /**PATH C:\laragon\www\bilaiGhor\resources\views\reseller\landing\product.blade.php ENDPATH**/ ?>