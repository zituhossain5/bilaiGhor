...
<head>
    ...
    <title><?php echo e($subcategory->subcategoryName ?? $subcategory->name); ?> | <?php echo e($landing->title); ?></title>
...
<body class="bg-gray-100 font-sans text-gray-800 pb-16 md:pb-0">
    ...
    <div class="container mx-auto px-4 py-6 flex flex-col lg:flex-row gap-6">
        ...
        <aside class="w-full lg:w-1/4 hidden lg:block bg-white shadow rounded-lg p-4 h-fit">
            ...
            <ul class="space-y-1 text-sm font-medium">
                <?php $__currentLoopData = $categories ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="border-b border-gray-100 last:border-0">
                        <div class="flex items-center">
                            <a href="<?php echo e($categoryRoute($cat)); ?>" class="flex-1 flex items-center py-3 hover:text-blue-600">
                            ...
                            <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                                <button type="button" onclick="toggleSubcat('cat-<?php echo e($cat->id); ?>', this)" class="p-2 text-gray-400 hover:text-blue-600">
                                    <i class="fa-solid fa-chevron-right text-xs cat-toggle-icon" style="<?php echo e($currentCatId == $cat->id ? 'transform:rotate(90deg)' : ''); ?>"></i>
                                </button>
                            <?php endif; ?>
                        </div>
                        <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                            <ul id="cat-<?php echo e($cat->id); ?>" class="ml-11 mb-2 space-y-0.5 <?php echo e($currentCatId == $cat->id ? '' : 'hidden'); ?>">
                                <?php $__currentLoopData = $cat->subcategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li><a href="<?php echo e($subcategoryRoute($sub)); ?>" class="block py-1.5 px-2 rounded hover:bg-blue-50 hover:text-blue-600 text-xs <?php echo e(($subcategory->id ?? null) == $sub->id ? 'text-blue-600 font-bold' : ''); ?>"><?php echo e($sub->subcategoryName ?? $sub->name); ?></a></li>
            ...
            <a href="<?php echo e(landing_url($landing->slug, '')); ?>" class="block mt-4 text-blue-600 font-bold text-sm">হোমে ফিরে যান <i class="fa-solid fa-angle-right ml-1"></i></a>
        </aside>

        
        <main class="flex-1">
            <div class="bg-white rounded-xl shadow p-4 md:p-6 mb-6">
                <h1 class="text-xl md:text-2xl font-bold text-gray-800 border-b-4 border-blue-600 inline-block pb-1"><?php echo e($subcategory->subcategoryName ?? $subcategory->name); ?></h1>
                <p class="text-sm text-gray-500 mt-2"><?php echo e($products->total()); ?> প্রোডাক্ট</p>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="bg-white rounded-lg shadow hover:shadow-xl transition p-3 relative">
                            <a href="<?php echo e(landing_url($landing->slug, 'product/'.$product->slug)); ?>"><img src="<?php echo e(asset($img)); ?>" alt="" class="w-full h-40 object-cover rounded mb-3"></a>
                            <h3 class="font-semibold text-sm line-clamp-2 mb-2"><?php echo e($product->name); ?></h3>
                            <div class="bg-blue-50 p-2 rounded mb-3 border border-blue-100">
                                <div class="flex justify-between"><span class="text-xs text-gray-600">মূল্য:</span><span class="font-bold text-blue-700">৳<?php echo e(number_format($displayPrice, 0)); ?></span></div>
                            </div>
                            <form action="<?php echo e(landing_url($landing->slug, 'cart/add')); ?>" method="POST">
                                <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
                <div class="bg-white rounded-xl shadow p-12 text-center text-gray-500">
                    ...
                    <a href="<?php echo e(landing_url($landing->slug, 'category/'.$subcategory->category->slug)); ?>" class="inline-block mt-4 text-blue-600 font-semibold">ক্যাটাগরিতে ফিরে যান</a>
                <?php else: ?>
                    <a href="<?php echo e(landing_url($landing->slug, '')); ?>" class="inline-block mt-4 text-blue-600 font-semibold">হোমে ফিরে যান</a>
                <?php endif; ?>
                </div>
            <?php endif; ?>
        </main>
        ...
        <ul class="py-2">
            <?php $__currentLoopData = $categories ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <li class="border-b">
                    ...
                    <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                        <button type="button" onclick="event.stopPropagation();toggleSubcat('mcat-<?php echo e($cat->id); ?>',this)" class="p-3"><i class="fa-solid fa-chevron-right text-xs" style="<?php echo e($currentCatId == $cat->id ? 'transform:rotate(90deg)' : ''); ?>"></i></button>
                    <?php endif; ?></div>
                    <?php if($cat->subcategories && $cat->subcategories->count() > 0): ?>
                        <ul id="mcat-<?php echo e($cat->id); ?>" class="bg-gray-50 <?php echo e($currentCatId == $cat->id ? '' : 'hidden'); ?>">
                            <?php $__currentLoopData = $cat->subcategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sub): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="fixed bottom-0 left-0 w-full bg-white border-t z-50 md:hidden py-2 flex justify-around text-gray-500 text-xs">
        ...
        <a href="<?php echo e(route('home')); ?>" class="flex flex-col items-center"><i class="fa-solid fa-store text-lg mb-1"></i>শপ</a>
...
<?php /**PATH C:\laragon\www\bilaiGhor\resources\views\reseller\landing\subcategory.blade.php ENDPATH**/ ?>