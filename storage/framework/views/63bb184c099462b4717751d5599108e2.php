...
<?php $__env->startSection('title','Manage Life Stages'); ?>
...
<div class="container-fluid">
    <div class="row mb-3 mt-3">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <h4 class="page-title mb-0" style="font-weight:700;color:#2d3436;">Product Life Stages</h4>
            <a href="<?php echo e(route('lifestages.create')); ?>" class="btn btn-primary rounded-pill shadow-sm px-4"><i class="fe-plus me-1"></i> Add Weight</a>
        </div>
    </div>
    <div class="row"><div class="col-12"><div class="card"><div class="card-body">
        <table id="datatable-buttons" class="table table-hover w-100 dt-responsive nowrap">
            <thead><tr><th style="width:50px;">SL</th><th>Life Stage Name</th><th>Sort Order</th><th>Status</th><th class="text-end" style="width:150px;">Action</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="attr-badge"><?php echo e($value->name); ?></span></td>
                    <td><?php echo e($value->sort_order ?? '—'); ?></td>
                    <td>
                        <?php if($value->status==1): ?><span class="badge badge-pill badge-soft-success">Active</span>
                        <?php else: ?><span class="badge badge-pill badge-soft-danger">Inactive</span><?php endif; ?>
                    </td>
                    <td class="text-end"><div class="d-inline-flex gap-2">
                        <?php if($value->status==1): ?>
                        <form method="post" action="<?php echo e(route('lifestages.inactive')); ?>" class="d-inline"><?php echo csrf_field(); ?><input type="hidden" value="<?php echo e($value->id); ?>" name="hidden_id"><button type="submit" class="action-btn btn-inactive" title="Deactivate"><i class="fe-eye-off"></i></button></form>
                        <?php else: ?>
                        <form method="post" action="<?php echo e(route('lifestages.active')); ?>" class="d-inline"><?php echo csrf_field(); ?><input type="hidden" value="<?php echo e($value->id); ?>" name="hidden_id"><button type="submit" class="action-btn btn-active" title="Activate"><i class="fe-eye"></i></button></form>
                        <?php endif; ?>
                        <a href="<?php echo e(route('lifestages.edit',$value->id)); ?>" class="action-btn btn-edit" title="Edit"><i class="fe-edit"></i></a>
                        <form method="post" action="<?php echo e(route('lifestages.destroy')); ?>" class="d-inline"><?php echo csrf_field(); ?><input type="hidden" name="hidden_id" value="<?php echo e($value->id); ?>"><button type="submit" class="action-btn btn-delete delete-confirm" title="Delete"><i class="fe-trash-2"></i></button></form>
                    </div></td>
                </tr>
...
<?php echo $__env->make('backEnd.layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\bilaiGhor\resources\views\backEnd\lifestage\index.blade.php ENDPATH**/ ?>